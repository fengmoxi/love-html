...
if(isset($_GET['add']) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if(preg_match('/^[\w\-]{3,30}$/',$textHref) == 0){
		$return = array ('status'=>0,'msg'=>'←链接名只能为3-30位的字母[a-zA-Z]、数字[0-9]、- 和 _ ');
	}else{		
		...
		if(file_exists(dirname(__FILE__) . "/just4u/$textHref.html")){
			$return = array ('status'=>0,'msg'=>'←这个链接已存在');
			$path_status = "失败：存在";
		}else{
			$path = dirname(__FILE__) . "/just4u/$textHref.html"; // 保存为
			$handle = fopen($path,"w");
			if($handle){
				$return = array ('status'=>1,'url'=>'http://'.$_SERVER['HTTP_HOST'].'/just4u/'.$textHref.'.html');
				$path_status = "成功：生成";
			}else{
